...
/// <summary>
/// An more performance alternative to the BCL LinkedList.
/// - Less CPU usage
/// - Allocation free (still it needs a buffer, like a normal List)
/// - No memory fragmentation
/// </summary>
/// <typeparam name="T"></typeparam>
public class IndexBasedLinkedList<T> : IEnumerable<T>, IReadOnlyCollection<T>
{
    private int _count;
    ...
    public IndexBasedLinkedList(int capacity = 4)
    {
        if (capacity < 1)
            capacity = 1;
    ...
    public ref Node this[int index]
    {
        get
        {
            ref Node node = ref _array[index];
            if (!node.used)
    ...
    /// <summary>
    /// O(1) contains, but based on index instead of T
    /// </summary>
    ...
    /// <summary>
    /// Iterate over list elements, from first to last
    /// </summary>
    public struct Enumerator : IEnumerator<T>
    {
        private readonly IndexBasedLinkedList<T> _list;
        private int _index;

        public Enumerator(IndexBasedLinkedList<T> list)
        {
            _list = list;
            _index = -2;
        }
